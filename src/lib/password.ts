...
// 등록/변경을 통과시키는 실제 하한(하드 게이트). 8자를 강제하지 않되 기본 안전선은 둔다(Supabase 기본 최소와 동일).
export const PW_FLOOR = 6;
// 강도 미터의 '권장' 기준 — 게이트가 아니라 점수 보너스·체크리스트 라벨용. 미터 UX를 위해 유지.
...
export function passwordError(pw: string, email?: string): string | null {
  if (pw.length < PW_FLOOR) return `비밀번호는 ${PW_FLOOR}자 이상이어야 합니다.`;
